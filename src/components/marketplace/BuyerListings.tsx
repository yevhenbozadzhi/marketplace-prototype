...
export function BuyerListings() {
  const { currentUser, filteredAssets, buyerProfile, sellers, openAsset, openChat } = useMarketplace();
        ...
        return (
          <article key={asset.id} className="rounded-md border border-[#ded6cc] bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-md bg-[#ecf6ed] px-3 py-1 text-sm font-semibold text-[#276437]">{match.score}% fit</span>
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-[#7b6b5d]">Seller: {seller?.company} · {match.reasons.join(", ")}</p>
              {seller && (
